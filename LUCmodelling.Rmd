---
title: "Spatial Analysis of Land-Use and Deforestation in CW Countries using Machine Learning"
subtitle: "Land cover classification in R - Banks Peninsula in New Zealand"
date: "6th April 2021"
...
This aims to show one (of many) workflows of how to perform a land cover classification in R using machine learning algorithms. 
The aim is to map the invasive species gorse (Ulex europaeus) on the Banks Peninsula in New Zealand. This plant was broad to New Zealand during the european settlement as a hedge plant and spreads into farmland since with negative consequences for the quality of the grassland. Each year millions of dollars are spent for it's control. To develop management strategies it's of importance to map the distribution of such plants. Since field sampling mean huge expenses, remote sensing methods are required for a spatially explicit monitoring. Sentinel satellite imagery from 2017 is going to be used to map the current distribution on a section of the Banks Peninsula.

The technical aim of this case study is to show land cover classifications based on satellite data can be performed in R using machine learning algorithms such as Random Forests.

### Method

The raster package will be used for processing of the satellite data as well as the caret package as a wrapper for the randomForest algorithm. Mapview is used for spatial visualization of the data. 
...
## ETL of Satellite Data

Loading the Sentinel data as well as a shape file of the training sites.
...
Getting an overview on the data. As satellite data, a Sentinel-2 scene from October 2017 is used. Sentinel has a spatial resolution of 10-20m and has spectral channels in the visible, near infrared as well as shortwave infrared. The Sentinel data subset for this case study contains the Sentinel channels 2-8 (visible and near infrared channels) as well as the NDVI and a yellowness index that was calculated as (red+green-blue)/(red+green+blue) as additional bands. Assume the yellowness index is valuable to distinguish the striking yellow color of the gorse from other vegetation.

The shape file contains the training sites of 9 Land cover classes. These are polygons (23 in total) that were digitized in QGIS on the basis of the Sentinel data using expert knowledge and can be regarded as a ground truth for the land cover classification. 
...
Using mapview's viewRGB function to visualize the Sentinel data as true color composite in the geographical context and overlay it with the polygons. Land cover class is assigned to respective polygons.
